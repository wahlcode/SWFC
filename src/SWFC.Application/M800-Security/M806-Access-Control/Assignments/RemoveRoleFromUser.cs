...
public sealed class RemoveRoleFromUserHandler : IUseCaseHandler<RemoveRoleFromUserCommand, bool>
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var roleAssignmentError = await _securityAdministrationGuard.ValidateRoleRemovalAsync(
            securityContext,
            command.UserId,
            role.Name.Value,
            cancellationToken);
        ...
        var changed = await _userRoleAssignmentWriteRepository.RemoveRoleAsync(
            command.UserId,
            command.RoleId,
            changeContext,
            cancellationToken);
        ...
        if (!changed)
        {
            return Result<bool>.Failure(
                new Error(
                    "m806.assignment.remove_role.failed",
                    "Role could not be removed from user.",
